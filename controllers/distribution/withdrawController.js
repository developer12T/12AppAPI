...
exports.checkout = async (req, res) => {
    try {
        ...
        const fromWarehouse = withdrawType === 'normal'
            ? shipping.warehouse?.normal
            : shipping.warehouse?.clearance

        if (!fromWarehouse) {
            return res.status(400).json({ status: 400, message: 'Invalid withdrawType or missing warehouse data!' })
        }

        const productIds = cart.listProduct.map(p => p.id)
        const products = await Product.find({ id: { $in: productIds } }).select('id name group brand size flavour weightGross weightNet listUnit')
        ...
        let listProduct = cart.listProduct.map(item => {
            ...
            if (!unitData) {
                return res.status(400).json({ status: 400, message: `Invalid unit for product ${item.id}` })
            }
...
exports.getOrder = async (req, res) => {
    try {
        ...
        if (!order) {
            return res.status(404).json({ status: 404, message: 'Distribution order not found!' })
        }
...
exports.updateStatus = async (req, res) => {
    try {
        ...
        if (order.status !== 'pending' && status !== 'canceled') {
            return res.status(400).json({ status: 400, message: 'Cannot update status, distribution is not in pending state!' })
        }
        ...
        if (status === 'canceled' && !orderId.endsWith('CC')) {
            ...
            if (isDuplicate) {
                let counter = 1
                while (await Distribution.findOne({ orderId: `${orderId}CC${counter}` })) {
                    counter++
                }
