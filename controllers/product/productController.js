...
exports.getFilters = async (req, res) => {
    try {
        ...
        const isEmptyRequest =
            ...
            (isEmptyArray(group) && isEmptyArray(brand) && isEmptyArray(size) && isEmptyArray(flavour))
        ...
        if (flavour && !isEmptyArray(flavour)) matchCondition.flavour = { $in: flavour }
        ...
        res.status(200).json({
            ...
            data: {
                ...
                flavour: attributes.length ? attributes[0].flavour : ['เลือกกลุ่มสินค้า']
            }
        })
...
exports.updateStatus = async (req, res) => {
    try {
        ...
        if (!id || !type || !status) {
            return res.status(400).json({ status: '400', message: 'id, type, status are required!' })
        }

        // if (type != 'sale' || type != 'refund' || type != 'withdraw'){
        //     return res.status(404).json({ status: '404', message: 'type is wrong!' })
        // }

        let product = {}
        if (type === 'sale') {
            product = await Product.findOneAndUpdate(
                { id }, { $set: { statusSale: status } }
            )
        } else if (type === 'refund') {
            product = await Product.findOneAndUpdate(
                { id }, { $set: { statusRefund: status } }
            )
        } else if (type === 'withdraw') {
            product = await Product.findOneAndUpdate(
                { id }, { $set: { statusWithdraw: status } }
            )
        }
