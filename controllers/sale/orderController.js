...
exports.checkout = async (req, res) => {
    try {
        const { type, area, storeId, routeId, note, latitude, longitude, shipping, payment } = req.body
        ...
        const products = await Product.find({ id: { $in: productIds } }).select('id name group brand size flavour listUnit')
...
exports.updateStatus = async (req, res) => {
    try {
        ...
        if (order.status !== 'pending' && status !== 'canceled') {
            return res.status(400).json({ status: 400, message: 'Cannot update status, order is not in pending state!' })
        }
