...
async function summaryOrder(cart) {
    try {
        ...
        const productIds = [
            ...
            ...(cart.listPromotion ? cart.listPromotion.flatMap((promo) => promo.listProduct.map(p => p.id)) : [])
        ]
...
async function summaryRefund(cart) {
    try {
        ...
        const enrichedRefunds = cart.listRefund.map(refundItem => {
            ...
            const unitData = productInfo.listUnit?.find(u => u.unit === refundItem.unit) || {}
        ...
        return {
            ...
            totalVat: (totalProduct - totalRefund - ((totalProduct - totalRefund) / 1.07)).toFixed(2),
